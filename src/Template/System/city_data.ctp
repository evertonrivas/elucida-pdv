...
<table class='table table-hover' id='tblResult'>
    <thead>
        <tr>
            ...
            <th>Nome</th>
            <th>UF</th>
            <th>C&oacute;digo IBGE</th>
            <th class='text-center last-column'>A&ccedil;&atilde;o</th>
        </tr>
    </thead>
    <tbody>
    <?php if($city_list->count()>0): ?>
    <?php foreach($city_list as $city): ?>
        <tr>
            <td><input type='checkbox' id='check_list[]' name='check_list[]' value="<?= $city->IDCIDADE; ?>"></td>
            <td><?= $city->NOME; ?></td>
            <td><?= $city->UF; ?></td>
            <td><?= $city->COD_IBGE; ?></td>
            <td><a href="<?php echo $this->Url->build('system/city_create/'.$city->IDCIDADE);?>" class="btn btn-light btn-sm" id="btnFilter"><i class="fas fa-edit"></i> Editar</a></td>
        </tr>
    <?php endforeach;?>
    <?php else: ?>
