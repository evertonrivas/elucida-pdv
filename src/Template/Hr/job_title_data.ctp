...
<table class='table table-hover' id='tblResult'>
    ...
    <?php foreach($data_list as $data): ?>
        <tr>
            <td><input type='checkbox' id='check_list[]' name='check_list[]' value="<?= $data->IDCARGO; ?>"></td>
            <td><?= $data->NOME; ?></td>
            <td><?= $data->CARGO_TIPO; ?></td>
            <td><a href="<?php echo $this->Url->build('hr/job_title_create/'.$data->IDCARGO);?>" class="btn btn-light btn-sm" id="btnFilter"><i class="fas fa-edit"></i> Editar</a></td>
        </tr>
    <?php endforeach;?>
    <?php else: ?>
