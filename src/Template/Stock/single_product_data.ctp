...
<table class='table table-hover' id='tblResult'>
    ...
    <?php foreach($product_list as $product): ?>
        ...
        <tr>
            ...
            <td class="text-right"><?php echo $inicio.$this->Number->currency($product->PRECO_COMPRA,"BRL").$fim; ?></td>
            <td class="text-right"><?php echo $inicio.$this->Number->currency($product->PRECO_VENDA,"BRL").$fim; ?></td>
            <td class="last-column"><a href="<?php echo $this->Url->build('stock/single_product_create/'.$product->IDPRODUTO);?>" class="btn btn-light btn-sm<?php echo $disabled; ?>" id="btnEdit"><i class="fas fa-edit"></i> Editar</a></td>
        </tr>
    <?php endforeach;?>
    <?php else: ?>
