		...
		<div class="row">
			...
			<div class="col">
				<div class="row justify-content-end">
					<select class="form-control form-control-sm" style="max-width: 170px!important;" id="cbStore" name="cbStore" required>
						<option value="" selected="">Selecione uma loja</option>
						<?php if(isset($stores)):?>
						<?php foreach($stores as $store):?>
						<option value="<?=$store->IDLOJA?>"><?=$store->NOME;?></option>
						<?php endforeach;?>
						<?php endif;?>
					</select>&nbsp;
...
function getData(){
	$.ajax({
        ...
        method: 'post',
        data: { PERIODO: $("#txtDate").val(), LOJA: $("#cbStore").val() },
