		...
		<div class="row">
			...
			<div class="col">
				<div class="row justify-content-end">
					<select class="form-control form-control-sm" style="max-width: 250px!important;" id="cbProvider" name="cbProvider" required>
						<option value="" selected="">Selecione um fornecedor</option>
						<?php if(isset($providers)):?>
						<?php foreach($providers as $provider):?>
						<option value="<?=$provider->IDFORNECEDOR?>"><?=$provider->FANTASIA;?></option>
						<?php endforeach;?>
						<?php endif;?>
					</select>&nbsp;
...
function getData(){
	$.ajax({
        ...
        method: 'post',
        data: { PERIODO: $("#txtDate").val(), FORNECEDOR: $("#cbProvider").val() },
        url: '<?=$this->Url->build("/report/stock_by_provider_data/")?>',
