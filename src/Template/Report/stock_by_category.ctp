		...
		<div class="row">
			...
			<div class="col">
				<div class="row justify-content-end">
					<select class="form-control form-control-sm" style="max-width: 250px!important;" id="cbCategory" name="cbCategory" required>
						<option value="" selected="">Selecione uma categoria</option>
						<?php if(isset($categories)):?>
						<?php foreach($categories as $category):?>
						<option value="<?=$category->IDCATEGORIA?>"><?=$category->NOME;?></option>
						<?php endforeach;?>
						<?php endif;?>
					</select>&nbsp;
...
function getData(){
	$.ajax({
        ...
        method: 'post',
        data: { PERIODO: $("#txtDate").val(), CATEGORIA: $("#cbCategory").val() },
        url: '<?=$this->Url->build("/report/stock_by_store_data/")?>',
